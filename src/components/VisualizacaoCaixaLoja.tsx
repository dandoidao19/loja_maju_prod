...
import { buildCumulativeSeries } from '@/lib/dateUtils'
import { useCaixaLojaData } from '@/hooks/useCaixaLojaData'

export default function VisualizacaoCaixaLoja({ titulo }: { titulo?: string }) {
  const { dados } = useDadosFinanceiros()
  const { data: caixaData, isLoading, error } = useCaixaLojaData()

  const [caixaReal, setCaixaReal] = useState(0)

  useEffect(() => {
    setCaixaReal(dados.caixaRealLoja || 0)
  }, [dados.caixaRealLoja])
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-1 space-y-1">
      <h2 className="font-semibold text-gray-800 text-sm">{titulo || 'Caixa Loja'}</h2>
        ...
        {isLoading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              ...
              <tbody>
                {caixaPrevisto.slice(0, 10).map((dia, idx) => (
                  <tr key={idx}>
