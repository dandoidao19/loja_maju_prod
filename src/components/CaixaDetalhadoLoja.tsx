...
export default function CaixaDetalhadoLoja({ onToggleTudo }: { onToggleTudo?: (mostrarTudo: boolean) => void }) {
  ...
  const { data: caixaPrevisto, isLoading: carregando, isSuccess } = useCaixaDetalhado('loja', {
    mostrando30Dias,
    mostrandoMes,
    mostrandoHistorico,
    mesFiltro,
  ...
  useEffect(() => {
    const caixaContexto = dados.caixaRealLoja
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-1 space-y-1" style={{ minWidth: 0 }}>
      <h2 className="font-semibold text-gray-800" style={{ fontSize: '12px' }}>Caixa Loja</h2>
