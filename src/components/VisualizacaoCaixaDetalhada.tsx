...
export default function VisualizacaoCaixaDetalhada({
  contexto,
  titulo,
  onToggleTudo
}: {
  ...
  // Para CASA o padrão deve ser 10 dias -> iniciar como false
  ...
  const lastLoadRef = useRef(0) // evita reloads muito próximos

  // ---------- helper fetchAll (paginação Supabase) ----------
  ...
  // inicializar mês atual (apenas para input tipo month)
  useEffect(() => {
    const hoje = new Date()
    const ano = hoje.getFullYear()
    const mes = String(hoje.getMonth() + 1).padStart(2, '0')
    setMesFiltro(`${ano}-${mes}`)
    console.log(`🎯 ${contexto} - Inicializado, modo: ${mostrando30Dias ? '30 DIAS' : mostrandoMes ? 'MÊS' : '10 DIAS'}`)
  }, [contexto])

  useEffect(() => {
    const caixaContexto = contexto === 'casa' ? dados.caixaRealCasa : dados.caixaRealLoja
  ...
  // CALCULAR ENTRADAS E SAÍDAS (garantir número) e log
  const calcularHoje = useCallback(async () => {
    try {
      ...
      if (contexto === 'casa') {
        ...
        console.log(`🎯 ${contexto} - Entradas hoje (casa): ${entradas}, Saídas hoje: ${saidas}`)
        ...
        console.log(`🎯 ${contexto} - Entradas hoje (loja): ${entradas}, Saídas hoje: ${saidas}`)
        setEntradasHoje(entradas)
        setSaidasHoje(saidas)
      }
    } catch (error) {
  ...
  const carregarCaixaPrevisto = useCallback(async () => {
    // Rate-limit / debounce reexecuções muito próximas
    const now = Date.now()
    if (now - lastLoadRef.current < 700) {
      console.log(`⏱️ ${contexto} - Ignorando recarga muito próxima (${now - lastLoadRef.current}ms)`)
      return
    }
    // Marca imediatamente para evitar duplicatas (resolve StrictMode/Double-invoke)
    ...
    // Só mostramos spinner quando não há dados exibidos (evita piscar)
    ...
    try {
      ...
      // substituir a lista exibida apenas quando o novo resultado estiver pronto
      setCaixaPrevisto(novoResultado)
      setUltimaAtualizacao(Date.now())
    } catch (error) {
      console.error(`❌ ${contexto} - Erro ao carregar caixa previsto:`, error)
      // mantemos a lista anterior para evitar piscar
    } finally {
      setCarregando(false)
      carregandoRef.current = false
    }
  }, [contexto, mostrando30Dias, mostrandoMes, mostrandoHistorico, mesFiltro, calcularDataNDias, fetchAll, normalizeDate, buildCumulativeSeries, caixaPrevisto])

  // EFEITOS: chamar cálculo de forma controlada
  useEffect(() => {
    console.log(`🔄 ${contexto} - Modo alterado: ${mostrando30Dias ? '30 DIAS' : mostrandoMes ? 'MÊS' : mostrandoHistorico ? 'TUDO' : '10 DIAS'}, mês: ${mesFiltro}`)
    calcularHoje()
    carregarCaixaPrevisto()
  }, [mostrando30Dias, mostrandoMes, mostrandoHistorico, mesFiltro, contexto, carregarCaixaPrevisto, calcularHoje])
  ...
  // estilos restritivos para evitar quebra
    ...
    if (contexto === 'casa') {
      return !mostrandoMes ? (
        <div style={botoesContainerStyle}>
          ...
          <button onClick={() => { setMostrando30Dias(true); setMostrandoMes(false); setMostrandoHistorico(false); onToggleTudo?.(false) }} disabled={carregando} className="px-1.5 py-0.5 bg-gray-500 text-white rounded text-xs font-medium">10 Dias</button>
        ...
        <div style={botoesContainerStyle}>
          ...
          <button onClick={handleVoltar30Dias} disabled={carregando} className="px-1.5 py-0.5 bg-gray-500 text-white rounded text-xs font-medium">10 Dias</button>
          <button onClick={handleMostrarHistorico} disabled={carregando} className="px-1.5 py-0.5 bg-green-500 text-white rounded text-xs font-medium">TUDO</button>
        </div>
      )
    } else {
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-1 space-y-1" style={{ minWidth: 0 }}>
      ...
      <div className="space-y-1">
        ...
        {carregando ? (
          <p className="text-gray-500 text-center py-2" style={{ fontSize: '12px' }}>Carregando {mostrando30Dias ? '30 dias' : mostrandoMes ? 'mês' : 'período'}...</p>
          ...
          <div className="text-center py-3">
            <p className="text-gray-500 text-xs">
              {contexto === 'casa' ? (mostrandoMes ? `Nenhuma transação encontrada para ${mesFiltro}` : 'Nenhuma transação nos próximos 10 dias') : (mostrando30Dias ? 'Nenhuma transação nos próximos 30 dias' : `Nenhuma transação encontrada para ${mesFiltro}`)}
            </p>
          </div>
        )}
      </div>
