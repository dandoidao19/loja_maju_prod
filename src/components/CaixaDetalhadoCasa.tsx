...
import { useCaixaDetalhado } from '@/hooks/useCaixaDetalhado'
...
export default function CaixaDetalhadoCasa() {
  ...
  const { data: caixaPrevisto, isLoading: carregando } = useCaixaDetalhado('casa', {
    mostrandoMes,
    mostrandoHistorico,
    mesFiltro,
  });
  ...
  useEffect(() => {
    const caixaContexto = dados.caixaRealCasa
  ...
  const renderBotoesModo = () => {
    if (!mostrandoMes && !mostrandoHistorico) {
        return (
    ...
    return (
        <div style={botoesContainerStyle}>
            <input type="month" value={mesFiltro} onChange={handleMesFiltroChange} disabled={carregando || !mostrandoMes} className="px-1.5 py-0.5 text-xs border border-gray-300 rounded" />
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-1 space-y-1" style={{ minWidth: 0 }}>
      <h2 className="font-semibold text-gray-800" style={{ fontSize: '12px' }}>Caixa Casa</h2>
